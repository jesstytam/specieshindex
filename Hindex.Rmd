...
Fetch <- function(genus, species) {
  library(rscopus)
  search <- scopus_search(query = "TITLE(\"", paste(genus), " ", paste(species), "\") AND DOCTYPE(ar OR re)",
...
H5 <- function(data, date) {
  library(dplyr)
  data$citations <- as.numeric(data$citations)
  sorteddf <- sort(data$citations, decreasing = TRUE) #sort in descending order
  sorteddf$cover_date <- as.Date(sorteddf$cover_date, format = "%Y-%m-%d") #change format of the date from factor to date
  sorteddf$year = format(sorteddf$cover_date, format="%Y")
  subsetdate <- subset(sorteddf, cover_date > as.date(paste(date)))
  h5 <- 0   #computes h-index
  for(i in 1:length(subsetdate)) {
    if (subsetdate[i] > h5) {
      h5 <- h5 + 1
    }
    }
...
Woylie <- Fetch(bettongia, penicillata)

Hxdf <- Hindex(xdf)
yearsxdf <- YearsPublishing(xdf)
Mxdf <- Mindex(xdf)
i10xdf <- i10(xdf)
h5xdf <- H5(xdf, 2016-07-01)
