...
FetchSpT <- function(genus, species, APIkey) {
  ...
  count <- searchCountT(genus, species, APIkey) #check the number of records
  ...
  start_record <- 0
  datalist = list()
  looprepeat <- ceiling(count/step_size) #the number of loop times, rounded up to the nearest integer
  #loop starts
  ...
  duplicates <- dim(searchcombine[duplicated(searchcombine$title),])[1] #check for duplicates
...
FetchSpTAK <- function(genus, species, APIkey) {
  ...
  count <- searchCountTAK(genus, species, APIkey) #check the number of records
  print(paste(count, "records found."))
  if (count > 1000) {
    print(paste("More than 1000 records found, this will take a while unless you have a nice computer."))
  }
...
SpH5 <- function(data) { #last 5 years from the current date
  ...
  d$year <- d$year-5
  as.Date(d)
  return(HAfterdate(data, d)) #calls HAfterdate function
}
```
...
SpHAfterdate <- function(data, date) {
  ...
  HAfterdate <- Hindex(subsetdata) #calls Hindex function
  return(HAfterdate)
}
```
...
```{r, h-index by year}
#this function calculates h-index by year
...
searchCountTAK("Bettongia", "penicillata", "442b9048417ef20cf680a0ae26ee4d86") #113
...
```{r, quokka}
#test done on 10 july
searchCountTAK("Setonix", "brachyurus", "442b9048417ef20cf680a0ae26ee4d86") #242
