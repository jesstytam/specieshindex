...
FetchSpTAKloop <- function(genus, species, APIkey) {
  ...
  search <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND DOCTYPE(ar OR re)"),
                          ...
                          start = 0,
                          wait_time = 1)
  returned <- as.numeric(length(search))
  if (returned > 500) {
    print(paste("More than 200 records found, fetching more records now."))
  }
  while (count > 500) { #loop
    search <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND DOCTYPE(ar OR re)"),
                          ...
                          start = returned,
                          wait_time = 1)
    print(paste(returned, "records retrived, fetching more records now."))
    if (returned == count) {
      break
    }
  }
...
Quokka <- FetchSpTAK("Setonix", "brachyurus", "442b9048417ef20cf680a0ae26ee4d86")
...
Platypus <- FetchSpTAK("Ornithorhynchus", "anatinus", "442b9048417ef20cf680a0ae26ee4d86")
...
Koala <- FetchSpTAK("Phascolarctos", "cinereus", "442b9048417ef20cf680a0ae26ee4d86")

TotalPub(Koala) #777
TotalCite(Koala) #14329
TotalJournals(Koala) #227
TotalArt(Koala) #748
TotalRev(Koala) #29
ARRatio(Koala) #96.2676962676963 : 3.73230373230373
...
```{r, rscopus}
#some tests
tiger <- FetchSpT("Panthera", "tigris", "442b9048417ef20cf680a0ae26ee4d86") #402 results
...
cat <- FetchSpTAKloop("felis", "catus", "442b9048417ef20cf680a0ae26ee4d86") #17622 results, error at 26%, current loop can only output 2000 records
elephant <- FetchSpTAKloop("Loxodonta","africana", "442b9048417ef20cf680a0ae26ee4d86") #1401 results
