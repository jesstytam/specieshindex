...
FetchSpY <- function(genus, species, APIkey) {
  ...
  step_size <- 2000 #the number of records to retrieve in each loop
  start_record <- 0
  datalist = data.frame()
  looprepeat <- ceiling(count/step_size)-1 #the number of loop times, rounded up to the nearest integer
  #loop starts
  for (i in 0:looprepeat) { 
    print(paste("starting iteration: ", i, " Note: iteration size is ", step_size, " records, which runs of 200 records inside each iteration."))
    print(paste("Fetching records now."))
    search <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\")&cursor=*&count=25 AND DOCTYPE(ar OR re)"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          max_count = step_size,
                          wait_time = 3)
...
```{r, fetch using title+abs+key}
#this function downloads citation data from scopus
FetchSpYNext <- function(genus, species, APIkey) {
  library(rscopus)
  library(rlang)
  library(dplyr)
    search1 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR AFT 2010"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)
    search1df <- entries_to_citation_df(search1$entries)
    search2 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR AFT 2000 AND PUBYEAR BEF 2010"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
    search2df <- entries_to_citation_df(search2$entries)
    search3 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR AFT 1980 AND PUBYEAR BEF 2000"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
    search3df <- entries_to_citation_df(search3$entries)
    search4 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR BEF 1980"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
    search4df <- entries_to_citation_df(search4$entries)
    datalist <- rbind(search1df, search2df, search3df, search4df)
  #loop ends
  returned <- dim(datalist)[1]
  print(paste(returned, "records retrived in total."))
  duplicates <- dim(datalist[duplicated(datalist$title),])[1] #check for duplicates
...
chimpsY <- FetchSpY("pan", "troglodytes", myAPI)
chimpsN <- FetchSpYNext("pan", "troglodytes", myAPI)
