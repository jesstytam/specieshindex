...
17. SpHYear
  + calculates the h-index by year
18. SpHGrowth
  + calculates the cumulative h-index
19. Allindices
...
```{r, fetch using title+abs+key}
#this function downloads citation data from scopus
FetchSpY <- function(genus, species, APIkey) {
  library(rscopus)
  library(rlang)
  library(dplyr)
  if (missing(APIkey)) {
  ...
  #search begins
  search1 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR > 2010"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)
  search1df <- entries_to_citation_df(search1$entries)
  search2 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR > 2000 AND PUBYEAR < 2011"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
  search2df <- entries_to_citation_df(search2$entries)
  search3 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR > 1990 AND PUBYEAR < 2001"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
  search3df <- entries_to_citation_df(search3$entries)
  search4 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR > 1980 AND PUBYEAR < 1991"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
  search4df <- entries_to_citation_df(search4$entries)
  search5 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR > 1970 AND PUBYEAR < 1981"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
  search5df <- entries_to_citation_df(search5$entries)
  search6 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR > 1960 AND PUBYEAR < 1971"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
  search6df <- entries_to_citation_df(search6$entries)
  search7 <- scopus_search(query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND PUBYEAR < 1961"),
                          api_key = paste0(APIkey),
                          verbose = TRUE,
                          wait_time = 3)  
  search7df <- entries_to_citation_df(search7$entries)
  datalist <- rbind(search1df, search2df, search3df, search4df, search5df, search6df, search7df)
  #search ends  
  returned <- dim(datalist)[1]
  print(paste(returned, "records retrived in total."))
  duplicates <- dim(datalist[duplicated(datalist$title),])[1] #check for duplicates
...
SpH5 <- function(data) { #last 5 years from the current date
  ...
  d$year <- d$year-5
  as.Date(d)
  return(SpHAfterdate(data, d)) #calls HAfterdate function
}
```
...
SpHYear <- function(data) {
  library(dplyr)
  data$year <- as.numeric(substr(data$cover_date, 1, 4))
  yeargroup <- data %>% #group data by year
    group_by(data$year)
  splitdf <- split(yeargroup, data$year) #split dfs into smaller dfs according to year
  splitH <- lapply(splitdf, function(splitdf){SpHindex(splitdf)}) #compute h-index for each year
  H <- setNames(stack(splitH)[2:1], c('year','h')) #convert list to df and adding column names
  H$year <- as.integer(levels(H$year))[H$year] #convert from factor to integer class
  fulldates <- seq(min(H$year), max(H$year)) #fill in the year gaps with no publications
  fulldates <- data.frame(year = fulldates) #create the df
  completeH <- merge(fulldates, H, by = "year", all.x = TRUE) #merge to get the complete time scale
  completeH[is.na(completeH)] <- 0 #replace NA values with 0
  return(completeH)
}
```

```{r, cumulative h-index}
...
Woylie <- FetchSpTAK("Bettongia", "penicillata", myAPI) #no duplicates
...
fox <- FetchSpTAK("Vulpes", "vulpes", myAPI) #3324 results
drosophila <- FetchSpTAK("Drosophila", "montana", myAPI) #93 results
drosophila2 <- FetchSpTAK("drosophila", "melanogaster", myAPI) #60904 results, error at 23%
drosophila3 <- FetchSpTAK("Mus", "musculus", myAPI) 

cat <- CountSpT("felis", "catus", myAPI) #17622 results, error at 26%, current loop can only output 2000 records
gnr_resolve(names = c("felis", "catus"))

chimps <- FetchSpTAK("pan", "troglodytes", myAPI) #10018 results
chimps2 <- FetchSpNext("pan", "troglodytes", myAPI)
