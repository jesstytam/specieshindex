...
CountSpT_scopus <- function(genus, species, synonyms, additionalkeywords, APIkey, datatype = "application/xml") {
  if (missing(APIkey)) {
...
#code from Fonti (modified)
CountSpTAK_scopus <- function(genus, species, synonyms, additionalkeywords, APIkey, datatype = "application/xml") {
  if (missing(APIkey)) {
    stop("You need to register for an API key on Scopus.") #stop running if API key missing
  }
  findname <- taxize::gnr_resolve(sci = c(genus, species)) #check if the species exist
...
FetchSpT_scopus <- function(genus, species, synonyms, additionalkeywords, language = 0, APIkey) {
...
#this function downloads citation data from scopus
FetchSpTAK_scopus <- function(genus, species, synonyms, additionalkeywords, language = 0, APIkey) {
...
CountSpT <- function(db, genus, species, synonyms, additinoalkeywords, language, size) {
  if (db == "scopus") {
    CountSpT_scopus(genus, species, synonyms, additionalkeywords, language)
  } else if (db == "wos") {
    CountSpT_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "base") {
    CountSpT_base(genus, species, synonyms, additionalkeywords)
  } else if (db == "lens") {
    CountSpT_lens(genus, species, synonyms, additionalkeywords, size)
  }
}
