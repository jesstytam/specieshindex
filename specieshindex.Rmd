...
```{r}
CountDomain <- function(genus, species, APIkey) {
  library(httr)
  library(XML)
  library(rlang)
  library(taxize)
  ...
  #conservation
  consURL <- GET("http://api.elsevier.com/content/search/scopus",
              query = list(apiKey = paste0(APIkey),
                           query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND conserv*"),
                           httpAccept = "application/xml")) #format the URL to be sent to the API
  stop_for_status(consURL) #pass any HTTP errors to the R console
  consData <- content(consURL, as = "text") #extract the content of the response
  newconsData <- xmlParse(consURL) #parse the data to extract values
  conservation <- as.numeric(xpathSApply(newconsData,"//opensearch:totalResults", xmlValue)) #get the total number of search results for the string
  #genomics
  geneURL <- GET("http://api.elsevier.com/content/search/scopus",
              query = list(apiKey = paste0(APIkey),
                           query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND (genom* OR gene*)"),
                           httpAccept = "application/xml")) #format the URL to be sent to the API
  stop_for_status(geneURL) #pass any HTTP errors to the R console
  geneData <- content(geneURL, as = "text") #extract the content of the response
  newgeneData <- xmlParse(geneURL) #parse the data to extract values
  genomics <- as.numeric(xpathSApply(newgeneData,"//opensearch:totalResults", xmlValue)) #get the total number of search results for the string
  #evolution
  evolURL <- GET("http://api.elsevier.com/content/search/scopus",
              query = list(apiKey = paste0(APIkey),
                           query = paste0("TITLE-ABS-KEY(\"",genus," ",species,"\") AND evolution*"),
                           httpAccept = "application/xml")) #format the URL to be sent to the API
  stop_for_status(evolURL) #pass any HTTP errors to the R console
  evolData <- content(evolURL, as = "text") #extract the content of the response
  newevolData <- xmlParse(evolURL) #parse the data to extract values
  evolution <- as.numeric(xpathSApply(evolData,"//opensearch:totalResults", xmlValue)) #get the total number of search results for the string
  #displaying all of the counts
  cat(
    conservation, "conservation publications", "\n",
    genemics, "genomics publications", "\n",
    evolution, "evolution publications"
  )
}
```
