...
Count <- function(db, search, level, genus, species, synonyms, additionalkeywords) {
  if (db == "scopus" && search == "t" && level == "species") {
    ...
    countsp <- CountGenusT_scopus(genus, species, synonyms, additionalkeywords)
  } else if (db == "wos" && search == "t" && level == "genus") {
    countsp <- CountGenusT_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "base" && search == "t" && level == "genus") {
    countsp <- CountGenusT_base(genus, species, synonyms, additionalkeywords)
  } else if (db == "scopus" && search == "tak" && level == "genus") {
    countsp <- CountGenusTAK_scopus(genus, species, synonyms, additionalkeywords)
  } else if (db == "wos" && search == "tak" && level == "genus") {
    countsp <- CountGenusTAK_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "base" && search == "tak" && level == "genus") {
    countsp <- CountGenusTAK_base(genus, species, synonyms, additionalkeywords)
  } 
  return(countsp)
}
```
...
```{r}
#fetch() wrapper
Fetch <- function(db, search, level, genus, species, synonyms, additionalkeywords) {
  if (db == "scopus" && search == "t" && level == "species") {
    fetchsp <- FetchSpT_scopus(genus, species, synonyms, additionalkeywords)
  } else if (db == "wos" && search == "t" && level == "species") {
    fetchsp <- FetchSpT_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "scopus" && search == "tak" && level == "species") {
    fetchsp <- FetchSpTAK_scopus(genus, species, synonyms, additionalkeywords)
  } else if (db == "wos" && search == "tak" && level == "species") {
    fetchsp <- FetchSpTAK_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "scopus" && search == "t" && level == "genus") {
    fetchsp <- FetchGenusT_scopus(genus, species, synonyms, additionalkeywords)
  } else if (db == "wos" && search == "t" && level == "genus") {
    fetchsp <- FetchGenusT_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "scopus" && search == "tak" && level == "genus") {
    fetchsp <- FetchGenusTAK_scopus(genus, species, synonyms, additionalkeywords)
  } else if (db == "wos" && search == "tak" && level == "genus") {
    fetchsp <- FetchGenusTAK_wos(genus, species, synonyms, additionalkeywords)
  } else if (db == "base") {
...
CountSpT(db = "base", genus = "Bettongia", species = "penicillata") #99
FetchGenusT(db = "scopus", genus = "Bettongia") #this works
...
httr::stop_for_status(response) #pass any HTTP errors to the R console
response_data <- XML::xmlParse(response) #working only on my linus pc ugh
resultCount <- as.numeric(XML::xpathSApply(response_data, "//response/result/@numFound"))

content_test <- jsonlite::fromJSON(rawToChar(response$content)) #content is xml so this isn't suppose to work i guess idk
content_test <- content(response, as = "text", encoding = "UTF-8") #YES

fromJSON(content_test, flatten = TRUE) #nope
