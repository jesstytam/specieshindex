...
CountSpT_base <- function(genus, species, synonyms, additionalkeywords) {
  ...
  } else {stop("Species missing from the Encyclopedia of Life.")}
...
CountSpT_wrap("scopus", genus = "Osphranter", species = "rufus") #4
CountSpT_wrap("scopus", genus = "Osphranter", species = "rufus", synonyms = "Macropus rufus") #118
CountSpT_wrap("scopus", genus = "Osphranter", species = "rufus", additionalkeywords = "conserv*") #0
CountSpT_wrap("scopus", genus = "Osphranter", species = "rufus", synonyms = "Macropus rufus", additionalkeywords = "conserv*") #1

CountSpT_wrap("wos", genus = "Osphranter", species = "rufus") #3
CountSpT_wrap("wos", genus = "Osphranter", species = "rufus", synonyms = "Macropus rufus") #116
CountSpT_wrap("wos", genus = "Osphranter", species = "rufus", additionalkeywords = "conserv*") #0
CountSpT_wrap("wos", genus = "Osphranter", species = "rufus", synonyms = "Macropus rufus", additionalkeywords = "conserv*") #1

test <- FetchSpT("scopus", genus = "Osphranter", species = "rufus", language = 1)



load("data/languages.rda")
write.csv(languages, "data/languages.csv")

#ggplot

test_bind <- list(Koala, Platypus, Quokka, Woylie)

test_bind <- test_bind %>% lapply(test_bind, function(x) mutate(sp = as.character(substitute(x))))
...
CountSpT_scopus_genus <- function(genus, synonyms, additionalkeywords, datatype = "application/xml") {
  findname <- taxize::gnr_resolve(sci = c(genus)) #check if the species exist
  if (nrow(findname)==0 | isFALSE("Encyclopedia of Life" %in% findname$data_source_title)) {
    stop(print(paste("Genus not found on the Encyclopedia of Life. Please check your spelling and try again.")))
  }
  ...
  if (isTRUE(findname_eol$user_supplied_name == findname_eol$matched_name)) {
    print(paste("Genus found on the Encyclopedia of Life."))
  } else {
    stop(print(paste("Genus not found on the Encyclopedia of Life. Please check your spelling and try again.")))
  }
  response <- httr::GET("http://api.elsevier.com/content/search/scopus",
...
CountSpT_scopus_genus("Macropus") #1101
CountSpT_scopus_genus("drosophila") #65802


#testing

FetchSpT("scopus", "macropus", "rufus")
...
CountSpT_scopus("Macropus", "rufus")
CountSpT_scopus("Bettongia", "penicillata")

test <- gnr_resolve(sci = "Macropus rufus", data_source_ids = list("1", "3", "4", "12"))
test <- taxize::gnr_resolve(sci = "Bettongia penicillata", data_source_ids = list("1", "3", "4", "12"))
test <- test[order(-test$score),]
if (test$score[1]>0.9) {
    print(paste("Species found on CoL, ITIS, NCBI, or EoL."))
  } else {
    print(paste("Species not found on CoL, ITIS, NCBI, or EoL. Please check your spelling and try again."))
  }

genus <- "Bettongia"
species <- "penicillata"

test <- taxize::gnr_resolve(sci = c(genus, species),
                            data_source_ids = list("1", "3", "4", "12"))
test <- test[order(-test$score),]
if (test$score[1]>0.9) {
    print(paste("Species found on CoL, ITIS, NCBI, or EoL."))
  } else {
...
test_woylie <- FetchSpT("scopus", "Bettongia", "penicillata")
test_kangaroo <- FetchSpT("scopus", "Macropus", "rufus")

wos_test <- FetchSpT("wos", "Sarcophilus", "harrisii")

wos_year <- getYear(wos_test, "Sarcophilus", "harrisii")
plotPub(wos_year)

test_woylie$year <- as.numeric(substr(test_woylie$cover_date, 1, 4))
output_by_year <- data.frame(table(test_woylie$year))
names(output_by_year)[names(output_by_year)=="Var1"] <- "Year"
output_by_year$Year <- as.numeric(as.character(output_by_year$Year))
output_by_year <- complete(output_by_year, Year = min(Year):max(Year), fill = list(Freq = 0))
output_by_year$spp <- "Bettongia penicillata"

output_by_year$Year <- as.numeric(output_by_year$Year)
output_by_year <- output_by_year %>% 
  complete(Year = min(Year):max(Year), fill = list(value1 = 0))



test_kangaroo$year <- as.numeric(substr(test_kangaroo$cover_date, 1, 4))
output_by_year_2 <- data.frame(table(test_kangaroo$year))
names(output_by_year_2)[names(output_by_year_2)=="Var1"] <- "Year"
output_by_year_2$spp <- "Macropus rufus"

combine_output <- rbind(output_by_year, output_by_year_2)

library(ggplot2)

ggplot(combine_output, aes(x = as.numeric(as.character(Year)),
                           y = Freq,
                           group = spp,
                           colour = spp)) +
  geom_line(size = 0.5,
...
plotPub(output_by_year)


getYear(test_woylie, "Bettongia", "penicillata")
plotPub(getYear(test_woylie, "Bettongia", "penicillata"))
