...
data.table::setDT(test)[, lapply(.SD, function(x) x[!is.na(x)][1L], by = ut)]
test_lapply <- data.frame(lapply(test, function(x) x[!is.na(x)]))
test[!is.na(test)]

setDT(test)[, name:= name[!is.na(name)][1L], by = ut]
