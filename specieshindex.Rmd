...
output_by_year$Year <- as.character(output_by_year$Year, format = "%Y")

library(ggplot2)

ggplot(output_by_year, aes(x = Year,
                           y = Freq)) +
  geom_line()
...
combine_test <- list(test_woylie, test_kangaroo)

class(combine_test[[1]]$cover_date)

lapply(combine_test$cover_date, FUN = function(x) as.numeric(substr(x, 1, 4)))

plotPub(combine_test)
```