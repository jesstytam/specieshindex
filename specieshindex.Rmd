...
Allindices <- function(data, genus, species) {
  if (all.equal(0, data$citations) == TRUE) {
    ...
    colnames(combine_sourcetype) <- c("genus_species", "species", "genus","publications", "citations", "journals", "articles", "reviews",
                           "years_publishing", "h", "m", "i10", "h5", names(SourceType(data)))
...
dat <- POST(url = "https://api.lens.org/scholarly/search",
            add_headers(.headers = c("Authorization" = "KeQCynn4V050fJZwOmaPIzUpAe7Fn5Y4rOkk79jb2IgUTSBwBBOb",
                                     "Content-Type" = "application/json")),
            body = create_query_string_T_lens(genus = "Macropus", species = "rufus", additionalkeywords = "conserv*")) #http 400
...
woylie_lens_T <- FetchSpT_lens(genus = "Bettongia", species = "penicillata", token = "KeQCynn4V050fJZwOmaPIzUpAe7Fn5Y4rOkk79jb2IgUTSBwBBOb")
woylie_lens_T_scopus <- FetchSpT(genus = "Bettongia", species = "penicillata", APIkey = "442b9048417ef20cf680a0ae26ee4d86")
woylie_lens_T_wos <- FetchSpT_wos(genus = "Bettongia", species = "penicillata")
...
Allindices(woylie_lens_T_wos, genus = "Bettongia", species = "penicillata")
st <- SourceType(woylie_lens_T)
st <- as.data.frame(t(st))
names(st) <- as.matrix(st[1,])
st_2 <- st[-1,]
