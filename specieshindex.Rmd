...
plotAllindices <- function(data) {
  ...
  #                                   common.legend = TRUE,
  #                                   legend = "right")
    combine_plot <- ggplot2::facet_wrap(h_plot, m_plot, i10_plot, h5_plot)
  return(combine_plot)
}
```
...
w$index <- factor(w$index,
                  levels = c("h", "m", "i10", "h5"),
                  labels = c("h-index", "m-index", "i10 index", "h5 index"))

ifelse(w$index=="m-index", round(w$value, 3), round(w$value, 0))

for (i in 1:length(w$genus_species)) {
  ifelse(w$index[i]=="m-index",
         w$value[i] <- round(w$value[i], 3),
         w$value[i] <- round(w$value[i], 0))
  print(paste(w$value[i]))
}

for (i in 1:length(w$genus_species)) {
  if (w$index[i]=="m-index"){
    w$value[i] <- round(w$value[i], 3)
  } else {
    w$value[i] <- round(w$value[i], 0)
  }
