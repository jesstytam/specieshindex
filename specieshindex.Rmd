...
```{r}
individualIndex <- function(data, y, title) {
  ifelse(w$index=="m-index", round(w$value, 3), round(w$value, 0))
  ...
  ggplot2::labs(title = paste0(title),
                colour = "Species") ++
  scale_y_continuous(labels = scales::label_number(drop0trailing = T)) +
    spindex_plot_theme()
  return(p)
}
```
...
plotAllindices <- function(data) {
  ifelse(data$index=="m-index", round(data$value, 3), round(data$value, 0))
  #h-index
  h_plot <- individualIndex(data, h, "h-index")
  #m-index
  m_plot <- individualIndex(data, m, "m-index")
  #i10
  i10_plot <- individualIndex(data, i10, "i10 index")
  #h5
  h5_plot <- individualIndex(data, h5, "h5 index")
  #combining the plots
...
plotAllindices(w)
individualIndex(w, h, "h-index")


#facet_warp

w_facet <- w %>% 
  pivot_longer(cols = h:h5,
               names_to = "index",
               values_to = "value")

w_facet$index <- factor(w_facet$index,
                        levels = c("h", "m", "i10", "h5"),
                        labels = c("h-index", "m-index", "i10 index", "h5 index"))

# ifelse(w_facet$index=="m-index", round(w_facet$value, 3), round(w_facet$value, 0))
# 
# for (i in 1:length(w_facet$genus_species)) {
#   ifelse(w_facet$index[i]=="m-index",
#          w_facet$value[i] <- round(w_facet$value[i], 3),
#          w_facet$value[i] <- round(w_facet$value[i], 0))
#   print(paste(w_facet$value[i]))
# }
# 
# for (i in 1:length(w_facet$genus_species)) {
#   if (w_facet$index[i]=="m-index"){
#     w_facet$value[i] <- round(w_facet$value[i], 3)
#   } else {
#     w_facet$value[i] <- round(w_facet$value[i], 0)
#   }
