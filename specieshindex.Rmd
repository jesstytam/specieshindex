...
FetchSpT_base <- function(genus, species, synonyms, additionalkeywords) {
  count <- CountSpT_base(genus, species, synonyms, additionalkeywords) #check the number of records
  print(paste(count, "records found."))
  if (count < 1) {
    noCitations <- data.frame(citations = 0)
    return(noCitations)
  ...
  results <- data.frame()
  step_size <- 125
  looprepeat <- ceiling(count/step_size)-1
  for (i in 0:looprepeat) { #number of loops to be determined by the count of the documents
    print(paste("loop", i+1))
    ...
    start_record <- as.numeric(summary(query)[1,1])
  }
  #renaming columns
  #names(results)[names(results) == "tot_cites"] <- "citations"
  #names(results)[names(results) == "dctype"] <- "description"
  #names(results)[names(results) == "dcdate"] <- "cover_date"
  #showing final list of records
  returned <- nrow(results)
  print(paste(returned, "records retrived in total."))
  return(results)
...
base_test <- bs_search(query = "dctitle:Phascolarctos cinereus", hits = 125) 
...
test <- httr::GET("https://api.base-search.net/cgi-bin/BaseHttpSearchInterface.fcgi",
                   query = list(func = "PerformSearch",
                                query = create_query_string_T_base(genus = "Phascolarctos", species = "cinereus")))
test_data <- XML::xmlParse(test)
resultCount <- as.numeric(XML::xpathSApply(test_data, "//response/result/@numFound"))

httr::content(test_data, as = "text")
xpathSApply(test_data, "//response/result/@doc")

CountSpT_base(genus = "Phascolarctos", species = "cinerius")
CountSpT_base(genus = "Phascolarctos", species = "cinereus")

CountSpT_base("Phascotos", "ci")


findname <- taxize::gnr_resolve(sci = c("Phascolarctos cinerius"))
if (findname$user_supplied_name[1] %in% findname$matched_name){
  print(paste("Species found on the Encyclopedia of Life."))
} else if (!findname$user_supplied_name[1] %in% findname$matched_name) {
  print(paste("Do you mean", findname$matched_name[findname$data_source_title == "Catalogue of Life"], "?"))
}

ifelse(findname$user_supplied_name[1] %in% findname$matched_name,
       print(paste("Species found on the Encyclopedia of Life.")),
       print(paste("Do you mean", findname$matched_name[findname$data_source_title == "Catalogue of Life"], "?")))



koala_test <- FetchSpT_base(genus = "Phascolarctos", species = "cinereus")

unique(koala_test$dctitle)
