...
An individual token is required to extract data from Lens. You will only be able to select the `Free Frial Scholarly API Plan` if your institution is not a subscriber of Lens. The token will only be available for 14 days, which can be renewed afterwards.
...
Multiple databases have been incorporated into `specieshindex`, including Scopus, Web of Science, and Lens. To differentiate between them, the suffix of the Count and Fetch functions have been labeled with the database's name, with Scopus being the exception.
