...
H-index is one of the main methods to measure publication influence objectively. Here, I have created a package to compute species h-index and other indices, to enable comparisons of the productivity of research on different species.
...
```{r, cache=TRUE, fig.height=6, fig.width=9, fig.align='center'}
Wyear <- SpHYear(Woylie)
filter(Wyear, Wyear$year > 1950)
Qyear <- SpHYear(Quokka)
filter(Qyear, Qyear$year > 1950)
Pyear <- SpHYear(Platypus)
filter(Pyear, Pyear$year > 1950)
Kyear <- SpHYear(Koala)
filter(Kyear, Kyear$year > 1950)
...
filter(Wgrowth, Wgrowth$year > 1950)
Qgrowth <- SpHGrowth(Quokka)
filter(Qgrowth, Qgrowth$year > 1950)
Pgrowth <- SpHGrowth(Platypus)
filter(Pgrowth, Pgrowth$year > 1950)
Kgrowth <- SpHGrowth(Koala)
filter(Kgrowth, Kgrowth$year > 1950)
