...
There are many ways to gauge research influence. We can simply sum up the number of publications or citations, which is a straight forward way without the need of any complicated mathematical formulae. On the other hand, there are more in-depth ways of quantifying research influence by using statistical indices. Some examples include the *h*-index, *m*-index, i10 index, h5 index, etc.
...
You can find the full list of functions [$\text{\underline{here}$](https://github.com/jessicatytam/specieshindex/blob/master/specieshindex_0.0.1.pdf).
...
Before initiating the retrieval of any data, make sure you have registered an API key from Scopus. Otherwise the functions will not run (and is in fact illegal to download the data). You can register your key [$\text{\underline{here}$](https://dev.elsevier.com/). Use your API key as the function parameter "myAPI".
